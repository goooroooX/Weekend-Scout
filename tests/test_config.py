...
def test_get_config_dir_returns_home_dir_for_global_install(tmp_path, monkeypatch):
    """When pyproject.toml is absent (global/site-packages install),
    get_config_dir() should fall back to ~/.weekend_scout."""
    import weekend_scout.config as cfg_module

    # Simulate site-packages layout: parents[1] has no pyproject.toml
    fake_pkg = tmp_path / "site-packages" / "weekend_scout"
    fake_pkg.mkdir(parents=True)
    monkeypatch.setattr(cfg_module, "__file__", str(fake_pkg / "config.py"))

    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", staticmethod(lambda: fake_home))

    assert cfg_module.get_config_dir() == fake_home / ".weekend_scout"
