...
def test_format_scout_message_city_events():
    ...
    assert "<b>1. Fest A</b>" in msg
    assert "<b>2. Fest B</b>" in msg
...
def test_format_scout_message_road_trips():
    ...
    assert "<b>01. Lodz Day Trip</b>" in msg
...
def test_format_scout_message_strips_prefixed_trip_name():
    ...
    assert "<b>01. Lodz Day Trip</b>" in msg
    assert "<b>01. A. Lodz Day Trip</b>" not in msg
...
def test_format_scout_message_strips_numeric_trip_prefix():
    ...
    assert "<b>01. Lodz Day Trip</b>" in msg
    assert "<b>01. 1. Lodz Day Trip</b>" not in msg
...
def test_format_scout_message_renders_all_trip_options():
    ...
    assert "<b>01. Trip 0</b>" in msg
    assert "<b>10. Trip 9</b>" in msg
    assert "<b>11. Trip 10</b>" in msg
    assert "<b>12. Trip 11</b>" in msg
...
def test_format_scout_message_renders_all_city_events():
    ...
    msg = format_scout_message("Warsaw", "2026-04-04", "2026-04-05", events, [])
    assert "<b>1. Fest 0</b>" in msg
    assert "<b>4. Fest 3</b>" in msg
...
def test_format_scout_preview_uses_markdownish_formatting():
    ...
    assert "_🌱 Scouted by Weekend Scout_" in preview
    assert "🔗 [Details](https://example.com/trip)" in preview
...
def test_format_scout_message_wraps_digest_with_stats_and_notes():
    from weekend_scout.telegram import DELIVERY_SEPARATOR, format_scout_message
    ...
    msg = format_scout_message(
        ...
        "2026-04-05",
        [_event()],
        [_trip(url="https://example.com/trip")],
        stats_lines=["Events found: 2 (1 new, 1 cached)"],
        notes_lines=["DEBUG INFORMATION", "Phase D skipped: all_confirmed"],
    )

    assert msg.index("Events found: 2 (1 new, 1 cached)") > msg.index("Scouted by Weekend Scout")
    assert msg.index("DEBUG INFORMATION") > msg.index("Scouted by Weekend Scout")
    assert msg.count(DELIVERY_SEPARATOR) == 2
    assert f"<i>Events found: 2 (1 new, 1 cached)</i>" in msg
    assert "<i>DEBUG INFORMATION</i>" in msg


def test_format_scout_message_stats_without_notes_uses_one_separator():
    from weekend_scout.telegram import DELIVERY_SEPARATOR, format_scout_message
    ...
    assert msg.count(DELIVERY_SEPARATOR) == 1
    assert f"<i>Events found: 2 (1 new, 1 cached)</i>" in msg


def test_format_scout_preview_wraps_digest_with_stats_and_notes():
    from weekend_scout.telegram import DELIVERY_SEPARATOR, format_scout_preview
    ...
    assert preview.index("Events found: 2 \\(1 new, 1 cached\\)") > preview.index("Scouted by Weekend Scout")
    assert preview.index("DEBUG INFORMATION") > preview.index("Scouted by Weekend Scout")
    assert preview.count(DELIVERY_SEPARATOR) == 2
    assert "<i>" not in preview
    assert "_Events found: 2 \\(1 new, 1 cached\\)_" in preview
    assert "_DEBUG INFORMATION_" in preview


def test_split_message_handles_long_composite_delivery_message():
    from weekend_scout.telegram import format_scout_message, split_message

    stats_lines = [f"Events found line {i}: " + ("A" * 120) for i in range(4)]
    notes_lines = [f"Debug line {i}: " + ("B" * 120) for i in range(4)]
    events = [
        _event(
            event_name=f"Festival {i}",
            description="C" * 110,
