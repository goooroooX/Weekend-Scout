...
python -m weekend_scout log-action --run-id "<run_id>" --action skip \
  --phase search --target-weekend "<saturday>" --detail '{"reason": "cached_only_requested"}'
```
#@ENDIF
#@IF codex
...
#@ENDIF
#@IF codex
Before writing the cities and events payloads, read `references/platform-codex.md`.
...
Write the covered city list JSON array to `cities_json_path` and the kept event array to `events_json_path`, then run:
