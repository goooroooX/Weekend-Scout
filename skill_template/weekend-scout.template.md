...
> payload files directly in `.weekend_scout/cache/` and name them `_tmp_<name>.tmp`;
> the CLI deletes those temp files automatically after a successful command. Reuse
...
#@ENDIF
#@IF codex
Create `detail_json_path` first using the Codex JSON file rule above, then run:
```bash
python -m weekend_scout log-action --run-id "<run_id>" --action skip \
...
- **Telegram not configured** (`telegram_bot_token` or `telegram_chat_id` blank in config):
  ...
  > ```
  > Then run `%%INVOKE_CMD%%` again to send this weekend's digest.
