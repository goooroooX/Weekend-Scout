...
def audit_run(config: dict[str, Any], run_id: str) -> dict[str, Any]:
    ...
    if run_complete_entry is None:
        ...
        if sent is False and send_reason not in {"telegram_not_configured", "send_failed"}:
        ...
        if sent is False and served_marked is not False:
    ...
    if events_saved_entry is not None and run_complete_entry is not None:
    ...
    if run_init_entry is not None and run_complete_entry is not None:
    ...
    if message_formatted_entry is not None and run_complete_entry is not None:
    ...
    if (run_complete_entry is not None and
            ...
            events_served_entry is None):
        errors.append("sent=true but no events_served entry was logged")
    if (run_complete_entry is not None and
            (run_complete_entry.get("detail") or {}).get("sent") is False and
            events_served_entry is not None):
        errors.append("events_served was logged even though sent=false")
    ...
    if message_formatted_entry is None:
        warnings.append("Missing message_formatted entry")
    if telegram_send_entry is None:
