...
> This document replaces `docs/weekend-scout-mvp-design.md` and reflects
> the current state of the project as of late March 2026.
> For the history of changes from the original design, see `docs/design_changes.md`.
...
- `COUNTRY_CODE_MAP`: ISO 3166-1 alpha-2 to full country name (27 countries)
- `COUNTRY_LANGUAGE_MAP`: full country name to 2-letter search language code

Supported: PL, DE, FR, CZ, SK, AT, HU, UA, LT, LV, EE, BY, IT, ES, PT,
NL, SE, NO, DK, FI, RO, HR, BG, RS, GR, TR, RU.
...
"Brandenburg"). Covers 3-5 major cities per supported country in both English
and native names (~200 entries). The `get_region()` function does a
...
**Date formatting:** `format_date_local()` formats ISO dates in 27 languages
with correct word order: `"28 marca 2026"` (Polish), `"28. Marz 2026"` (German),
...
Weekend-Scout/
    ...
    docs/                                  Documentation
        ...
        weekend-scout-mvp-design.md        Original design (historical)
        design_changes.md                  Deviation log
...
Lithuania, Latvia, Estonia, Belarus, Italy, Spain, Portugal, Netherlands,
Sweden, Norway, Denmark, Finland, Romania, Croatia, Bulgaria, Serbia, Greece,
Turkey, Russia.
